...
{
    /// <summary>
    /// This is exactly the same as the 3pl model except that it uses the 1.7 constant.
    /// </summary>
    class IRTModel3pln : IRTModel
    {
        public IRTModel3pln()
            : base(IRTModelFactory.Model.IRT3PLn)
        {
        }

        public override IRTModel DeepCopy()
        {
            IRTModel3pln copy = (IRTModel3pln)(this.MemberwiseClone());
            return copy;
        }
        ...
        public override void SetParameterCount(int parameterCount, int scorePoints)
        {
            ...
            if (parameterCount != 3)
                throw new Exception("ScoringEngine.MeasurementModels.IRTModel.SetParameterCount. The 3PLn model must have 3 parameters not " + parameterCount.ToString());
            _parameterCount = parameterCount;
            _parameters = new double[_parameterCount];
        }

        public override void SetParameter(int position, double value)
        {
            if ((_parameterCount < position) || (_parameters == null) || (position < 0))
                throw new Exception("ScoringEngine.MeasurementModels.IRTModel.SetParameter. Cannot set.");
            if (position == 0)
                _parameters[position] = 1.7*value;
            else
        ...
        public override void PrintDebugInfo(string itemName, double score, double theta, string filename)
        {
            ...
            tw.WriteLine(itemName + ",IRT3pln," + score.ToString() + "," + theta.ToString() + "," + ComputeProbability(score, theta) + "," + D1LnlWrtTheta(score, theta) + "," + D2LnlWrtTheta(score, theta) + ",a=" + _parameters[0].ToString() + ",b=" + _parameters[1].ToString() + ",c=" + _parameters[2].ToString());
            tw.Close();
        }
