...
*/
        ...
        public static DateTime ConvertEST_XST(DateTime sourceDateTime, int offset)
        {
            ...
            return sourceDateTime.AddHours(offset * -1);
        }

        public static DateTime ConvertXST_EST(DateTime sourceDateTime, int offset)
        {
            //int offset = AppConfigSingleton.Instance.TimeZoneOffset;
            //NOTE: Larry wants to ignore the daylight saving
            //if (DateTime.Today.IsDaylightSavingTime())
            ...
            return sourceDateTime.AddHours(offset);
        }
        public static DataTable ConvertValuesEST_XST(DataTable sourceTable, int offset)
        {
            if (sourceTable == null)
                return null;
            ...
            {                
                if(sourceTable.Columns[col].DataType != typeof(DateTime))
                    continue;
                for(int row=0; row<rows; row++)
                {
                    if(sourceTable.Rows[row][col] != DBNull.Value)
                        sourceTable.Rows[row][col] = ConvertEST_XST((DateTime)sourceTable.Rows[row][col], offset);
                }
            }
            return sourceTable;
        }
