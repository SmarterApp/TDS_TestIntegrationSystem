...
/*
 Ported by Mike Mariano based on .NET 4.0. 
 */
...
{
    /// <summary>
    /// This is a port of the internal StaticFileHandler for .NET 4.0 except you can override the file path.
    /// Also HTTP ranges are supported.
    /// </summary>
    internal class StaticFileHandler3 : IHttpHandler 
	{
        ...
        private static FileInfo GetFileInfo(string physicalPath) 
        {
            // Check whether the file exists
            if (!File.Exists(physicalPath)) 
            { 
                throw new HttpException(HttpStatus.NotFound, SR.GetString("File_does_not_exist"));
            }
            ...
            {
                throw new HttpException(HttpStatus.NotFound, SR.GetString("Error_trying_to_enumerate_files"), ioEx);
            } 
            catch (SecurityException secEx) 
            {
                throw new HttpException(HttpStatus.Unauthorized, SR.GetString("File_enumerator_access_denied"), secEx);
            } 

            // To be consistent with IIS, we won't serve out hidden files 
            if ((((int)fileInfo.Attributes) & ((int)FileAttributes.Hidden)) != 0) 
            { 
                throw new HttpException(HttpStatus.NotFound, SR.GetString("File_is_hidden")); 
            }
            ...
            if ((((int)fileInfo.Attributes) & ((int)FileAttributes.Directory)) != 0) 
            {
                ...
                {
                    // Just return 403 
                    throw new HttpException(HttpStatus.Forbidden, SR.GetString("Missing_star_mapping"));
                }
                else 
                { 
                    // Redirect to a slash suffixed URL which will be
                    // handled by the #1# handler mapper 
        ...
        private static bool GetNextRange(string rangeHeader, ref int startIndex, long fileLength, out long offset, out long length, out bool isSatisfiable) 
        {
            ...
            if ((startIndex < rangeHeader.Length) != true) throw new ArgumentException("startIndex < rangeHeader.Length");
        ...
        private static void SendFile(string physicalPath, long offset, long length, long fileLength, HttpContext context) 
        {
            ...
            catch (ExternalException e) 
            {
                ...
                {
                    throw new HttpException(HttpStatus.Unauthorized, SR.GetString("Resource_access_forbidden")); 
                }
                throw; 
            } 
        }
